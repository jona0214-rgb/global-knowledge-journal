...
API 응답에서는 필수 섹션 11개를 ID별 고정 키로 생성하므로 섹션 중복·누락을
스키마 단계에서 차단합니다. 인용은 URL을 중복 작성하지 않고 6개 출처 중 하나의
`source_key`를 선택하며, 저장 전에 코드가 해당 출처의 제목과 URL을 연결합니다.
내용·분량 검증이 실패하면 정확한 오류를 다음 생성 요청에 전달해 최대 2회
재생성하고, 모든 검증을 통과하기 전에는 결과물을 공개하지 않습니다.
